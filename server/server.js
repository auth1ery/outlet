...
app.post('/api/verify', async (req, res) => {
  try {
    ...
    const pending = await redis.get(`pending:${email}`);
    if (!pending) {
      return res.status(400).json({ error: 'registration expired' });
    }

    const { username, hash } = JSON.parse(pending);
...
io.on('connection', async (socket) => {
  const userRows = await sql`SELECT id, username, email FROM users WHERE id = ${socket.userId}`;
  if (userRows.length === 0) return socket.disconnect();

  const user = userRows[0];
  users.set(socket.userId, { username: user.username, socketId: socket.id });

  io.emit('user_status', { userId: socket.userId, username: user.username, status: 'online' });
  io.emit('online_users', Array.from(users.values()).map(u => u.username));
  ...
  socket.on('typing', async (typing) => {
    ...
    socket.broadcast.emit('typing', { userId: socket.userId, username: user.username, typing });
  });
